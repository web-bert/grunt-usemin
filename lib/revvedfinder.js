...
//
// Build a regexp that will allow to check whether the found revved dirs
// are matching the given requirements (search directories and directory where
// the original is located)
//
var buildDirectoriesRegexp = function(searchDirs, filedir) {
  var searchString;

  if ((searchDirs.length == 1) && (searchDirs[0] === '.') && (filedir === '.')) {
    searchString = '\.';
  } else {
    searchString = '(' + searchDirs.map(function(d) { return regexpQuote(d)}).join('|') + ')/' + filedir;
  }
...
//
// Build a search string for the finder (for example expandfn)
//
var buildSearchString = function(searchDirs, filedir) {
  var searchString;

  if (searchDirs.length == 1) {
    searchString = searchDirs[0] + '/**/' + filedir + '/*';
  } else {
    searchString = '{' + searchDirs.join(',') + '}/**/' + filedir + '/*';
  }
