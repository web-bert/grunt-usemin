...
//
//
var ConfigWriter = module.exports = function ( flow, postprocessors_list, dirs ) {
  var self = this;
  this.flow = flow;
  ...
  flow.forEach(function(item) {
    var step = require(path.join(__dirname, 'config', item));
    self.steps.push(step);
  });

  postprocessors_list.forEach(function(item) {
    var pp = require(path.join(__dirname, 'config', item));
...
//    (for example the requirejs configuration)
